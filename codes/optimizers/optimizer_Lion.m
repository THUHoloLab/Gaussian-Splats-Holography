classdef optimizer_Lion < handle
    ...
    methods
        function self = optimizer_Lion(beta1,beta2,epsilon)
...
function [step, avg_g, avg_gsq] = lion_step(g, avg_g, avg_gsq, learnrate, beta1, beta2, epsilon)
% lionstep   Calculate lion update step for a single parameter
...
if beta1~=1
    ...
    avg_g = beta1 .* avg_gsq + (1 - beta1) .* g;
end

step = -learnrate.*( sign(avg_g + epsilon) );

avg_gsq = beta2 .* avg_gsq + (1 - beta2) .* g;
end
...
if ~(isreal(x) || ~isnumeric(x)) || ~(isreal(y)||~isnumeric(y)) || ~(isreal(z)||~isnumeric(z))
    error(message('nnet_cnn:solver:complexGradientlion'));
end
end
