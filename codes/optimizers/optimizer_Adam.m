classdef optimizer_Adam < handle
    properties
        mom1
        mom2
        beta1
    ...
    methods
        function self = optimizer_Adam(beta1,beta2,epsilon)
...
function [step, avg_g, avg_gsq] = adam_step(g, avg_g, avg_gsq, learnrate, beta1, beta2, epsilon)
...
if beta1~=1
    ...
    avg_g = (beta1./(1 - beta1)).*avg_g;
    avg_g = (1 - beta1).*(avg_g + g);
end
...
if ~(isreal(x) || ~isnumeric(x)) || ~(isreal(y)||~isnumeric(y)) || ~(isreal(z)||~isnumeric(z))
    error(message('nnet_cnn:solver:complexGradientAdam'));
end
end
